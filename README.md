...
> 版本：0.2.17 · 安装包见 [Releases](../../releases)
...
| **整本持久缓存** | 开启后阅读时后台沿"下一页"链把**后续全部章节**依次抓取入库（扩展源 IndexedDB，约 0.5–0.8s/页温和限速）：重开/续读秒加载零流量，站点限流或断网时照常翻章；中断自动断点续抓（从上次链尾继续）；顶部栏实时显示「缓存中 N」，抓完整本提示「已缓存全书」；分页式站点的章末短分页（一两百字）正常入库；设置面板可按书删除或清空全部缓存 |
...
| **书架** | 独立全页书架统一管理全部书的阅读进度与整本缓存：按书名搜索、「全部/在读/已缓存」筛选；每本书一张卡，显示读到的章节与百分比、最近阅读时间与缓存章数徽标，一键续读回上次位置；同书多条进度（书键漂移）自动归并为一本书，移除时可选同时清掉整本缓存；弹窗「书架 ›」进入 |
...
# 11. 书架全页回归（入口/空态/书键漂移合并/续读/仅缓存书/移除清缓存，33 项断言）
...
src/content/
  ...
  next-chapter.js           预加载器：fetch + 编码探测 + 解析缓存 + 熔断限速 + 整本链（bookSaver）
  chapter-cache.js          整本持久缓存：IndexedDB 读写消息封装、微批落库、断点续抓
...
src/popup/                  工具栏弹窗
src/bookshelf/              书架全页（html/css/js：阅读进度与整本缓存的统一管理入口）
